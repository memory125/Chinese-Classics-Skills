...
try:
    ...
    print("⚠️ 未安装 transformers，使用规则翻译 fallback")
...
class ClassicalChineseTranslatorV2:
    ...
    def __init__(self, use_rule_based=True, use_llm=False):
        """初始化翻译器
        
        Args:
            use_rule_based: 是否启用规则翻译 (默认 True)
            use_llm: 是否启用 LLM 翻译 (需要配置 API key)
        """
    ...
    def translate_with_llm(self, classical_text: str) -> Optional[str]:
        """使用 LLM 进行文言文翻译"""
        
        if not self.use_llm or not hasattr(self, 'client'):
            return None
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system", 
                        "content": "你将文言文翻译成现代汉语，保持原意，语言通俗易懂。不要添加额外解释，只输出翻译结果。"
                    },
    ...
    def translate(self, classical_text: str) -> str:
        ...
        # 如果规则翻译失败，尝试 LLM
        if self.use_llm:
            llm_result = self.translate_with_llm(classical_text)
            if llm_result:
                return llm_result
        
        # fallback: 返回原文 + 提示
        return f"[需要配置 API key] {classical_text}"
...
if __name__ == "__main__":
    ...
    print("=== 方案 A: 规则翻译 ===")
    translator_rule = ClassicalChineseTranslatorV2(use_rule_based=True, use_llm=False)
    
    test_text = "刘豫州王室之胄，英才盖世"
    annotated = translator_rule.format_with_annotations(test_text)
    translated = translator_rule.translate(test_text)
    
    print(f"原文：{test_text}")
    print(f"注音：{annotated}")
    print(f"译文：{translated}")
    
    # 方案 B: LLM 翻译 (需要配置 API key)
    print("\n=== 方案 B: LLM 翻译 ===")
    translator_llm = ClassicalChineseTranslatorV2(use_rule_based=False, use_llm=True)
    
    translated_llm = translator_llm.translate(test_text)
    print(f"译文：{translated_llm}")
