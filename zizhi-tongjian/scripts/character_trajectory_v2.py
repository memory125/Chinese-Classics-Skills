...
class CharacterTrajectoryGeneratorV2:
    """人物履历生成器 v2.0 (增强版)"""
    ...
    # 身份关键词映射 (增强版)
    IDENTITY_KEYWORDS = {
        '皇帝/君主': ['帝', '皇', '王', '陛下', '天子', '寡人', '孤'],
        '将军/统帅': ['将', '帅', '军', '大将军', '都督', '兵'],
        '谋士/丞相': ['相', '宰', '谋', '策', '师', '臣光曰', '先生'],
        '诸侯/藩王': ['侯', '公', '伯', '封'],
        '商人/平民': ['商', '民', '贩', '徒', '布衣'],
    }
    ...
    def generate_profile(self, character_name: str) -> Dict:
        ...
        # 8. 判断角色类型 (新增功能)
        role_type = self._determine_role(timeline, character_name)
    ...
    def _determine_role(self, timeline: List[Dict], character_name: str) -> str:
        """判断人物角色类型 (新增功能)"""
        ...
        for event in timeline:
            ...
            for role, keywords in self.ROLE_KEYWORDS.items():
                score = sum(1 for kw in keywords if kw.lower() in wisdom.lower())
...
if __name__ == "__main__":
    ...
    # 测试：生成刘邦人物档案 (增强版 v2.0)
    print("=== 测试：刘邦人物档案 (增强版 v2.0) ===")
    profile = generator.generate_profile("刘邦")
    
    if 'error' not in profile:
        print(f"姓名：{profile['name']}")
        print(f"时期：{profile['period']}")
        print(f"身份：{profile['identity']} ← **增强版!**")
        print(f"角色类型：{profile['role_type']} ← **新增功能!**")
